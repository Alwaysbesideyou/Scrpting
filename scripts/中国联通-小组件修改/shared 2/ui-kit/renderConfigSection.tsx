...
type RenderConfigSectionProps = {
  ...
  setMediumHourlyBarBaseMB: (v: number) => void
...
const MEDIUM_HOURLY_BAR_BASE_OPTIONS = [
    ...
    <Section
      ...
      footer={
        <Text font="caption2" foregroundStyle="secondaryLabel">
          ...
          {"\n"}• 中号柱状图基准：默认 500MB；若某小时超过基准，则按当天已出现的最大小时用量缩放。
        </Text>
      }
    >
      <Toggle
      ...
      <Picker
        title={"中号每小时柱状图基准"}
        value={mediumHourlyBarBaseMB}
        onChanged={(value: number) => {
          const n = Number(value)
          setMediumHourlyBarBaseMB(Number.isFinite(n) ? n : 500)
        }}
        pickerStyle={"menu"}
      >
        {MEDIUM_HOURLY_BAR_BASE_OPTIONS.map((opt) => (
          <Text key={opt.value} tag={opt.value as any}>
            {opt.label}
          </Text>
        ))}
