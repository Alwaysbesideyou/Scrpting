...
async function fetchCookieFromBoxJsForSettings(boxJsUrl: string): Promise<string | null> {
  const boxKey = "ComponentService"
  const base = String(boxJsUrl || defaultChinaUnicomSettings.boxJsUrl || "https://boxjs.com").trim().replace(/\/$/, "")
  if (!base) return null

  try {
    const response = await fetch(`${base}/query/data/${boxKey}`, { headers: { Accept: "application/json" } })
...
function SettingsView() {
  ...
  const initialMediumHourlyBarBaseMB =
    typeof initial.mediumHourlyBarBaseMB === "number" && Number.isFinite(initial.mediumHourlyBarBaseMB)
      ? initial.mediumHourlyBarBaseMB
      : (defaultChinaUnicomSettings.mediumHourlyBarBaseMB ?? 500)
  ...
  const [isFetchingCookie, setIsFetchingCookie] = useState<boolean>(false)
  ...
  const [mediumHourlyBarBaseMB, setMediumHourlyBarBaseMB] = useState<number>(initialMediumHourlyBarBaseMB)
  ...
  const handleFetchCookieFromBoxJs = async () => {
    const effectiveBoxJsUrl = String(boxJsUrl || defaultChinaUnicomSettings.boxJsUrl || "https://boxjs.com").trim()
    if (!effectiveBoxJsUrl) {
      try {
        await Dialog?.alert?.({ title: "BoxJs 地址为空", message: "请先填写 BoxJs 地址。", buttonLabel: "知道了" })
      } catch { }
      return
    }
    ...
    try {
      const fetchedCookie = await fetchCookieFromBoxJsForSettings(effectiveBoxJsUrl)
      if (!fetchedCookie) {
        await Dialog?.alert?.({
          title: "未获取到 Cookie",
          message: "请确认 BoxJs 已运行，并且 ComponentService 中存在 ChinaUnicom.Settings.Cookie。",
          buttonLabel: "知道了",
        })
        return
      }
      ...
      setEnableBoxJs(true)
      setBoxJsUrl(effectiveBoxJsUrl)

      const latest = loadChinaUnicomSettings()
      saveChinaUnicomSettings({
        ...latest,
        cookie: fetchedCookie,
        enableBoxJs: true,
        boxJsUrl: effectiveBoxJsUrl,
      })

      await Dialog?.alert?.({
        title: "已保存 Cookie",
        message: "已从 BoxJs 获取 Cookie，并保存到登录凭证的手动填写位置。",
        buttonLabel: "完成",
      })
    } catch (e) {
      const msg =
        e && typeof e === "object" && "message" in e ? String((e as { message?: unknown }).message ?? e) : String(e)
      try {
        await Dialog?.alert?.({ title: "获取失败", message: msg, buttonLabel: "知道了" })
      } catch { }
    } finally {
    ...
    <NavigationStack>
      ...
      >
        ...
        <Section
          ...
          footer={
            <Text font="caption2" foregroundStyle="secondaryLabel">
              这里既可以手动粘贴 Cookie，也可以点击下方按钮从 BoxJs 自动获取并回填到同一位置。
            </Text>
          }
        >
          <TextField
          ...
          />
          <Button
            title={isFetchingCookie ? "正在获取 Cookie..." : "从 BoxJs 获取并回填 Cookie"}
            systemImage="arrow.down.doc"
            action={handleFetchCookieFromBoxJs}
          />
        </Section>
        ...
        <RenderConfigSection
          ...
          mediumHourlyBarBaseMB={mediumHourlyBarBaseMB}
          setMediumHourlyBarBaseMB={setMediumHourlyBarBaseMB}
          includeDirectionalInTotal={includeDirectionalInTotal}
