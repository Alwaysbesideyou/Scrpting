...
export function buildAiUserPrompt(shortcutInput: ShortcutInput, sourceText: string, config?: AlertpilotConfig): string {
    const rawText = shortcutInput.rawText || ""
    const webTitle = shortcutInput.webTitle || ""
    const inputUrl = shortcutInput.inputUrl || ""
    const userProfilePrompt = config ? formatUserPreferencePrompt(config, sourceText || rawText || webTitle || "") : ""
...
export async function generateAiItemsForShortcutInput(
    ...
    const prompt = buildAiUserPrompt(shortcutInput, sourceText, config)
