...
import { deleteClipboardClipsByRange, deleteAllClipboardClips, deleteClip, deleteFavoriteClips, deleteTextClipsByContent, findClipByHash, findTextClipsByContent, insertClip, listClipGroups, listClips, listImagePaths, trimActiveClips, updateClipContent, updateClipState, updateClipTitle as updateClipTitleRow, getFullClipContent, getClipById, listSpotlightClips } from "./database"
import { imageContentHash, removeImage, saveImageForClip } from "./image_store"
import { bumpClipDataVersion } from "./change_signal"
import { deleteClipFromSpotlight, indexClipForSpotlight, rebuildSpotlightIndex } from "../services/spotlight_index"

function syncSpotlightClip(item: ClipItem): void {
  void indexClipForSpotlight(item).catch(() => {})
}

function removeSpotlightClip(id: string): void {
  void deleteClipFromSpotlight(id).catch(() => {})
}

function rebuildSpotlightClips(): void {
  void rebuildSpotlightIndex().catch(() => {})
}
...
export async function addClipFromPayload(payload: ClipPayload, settings: CaisSettings): Promise<CaptureResult> {
  ...
  if (existing) {
    ...
    bumpClipDataVersion()
    syncSpotlightClip({ ...existing, updatedAt: now })
...
export async function clearAllClipboardClips(): Promise<void> {
  const imagePaths = await listImagePaths()
  await deleteAllClipboardClips()
  for (const path of imagePaths) await removeImage(path)
  bumpClipDataVersion()
  rebuildSpotlightClips()
}

export async function deleteClipboardTextClipsByContent(content: string): Promise<number> {
  const fixedContent = normalizeClipContent(content)
  if (!fixedContent.trim()) return 0
  const removed = await deleteTextClipsByContent(fixedContent)
  if (removed > 0) bumpClipDataVersion()
  return removed
}
...
export { getFullClipContent, getClipById, listSpotlightClips }
