...
const ignoredPasteboardChangeCounts = new Set<number>()

async function rememberSelfWrite(text?: string): Promise<void> {
  if (text != null) {
    lastSelfWriteText = text
    lastSelfWriteAt = Date.now()
  }
  const changeCount = await currentChangeCount().catch(() => 0)
  if (changeCount > 0) {
    ignoredPasteboardChangeCounts.add(changeCount)
    // 只保留最近少量自写入 changeCount，避免集合无限增长。
    if (ignoredPasteboardChangeCounts.size > 20) {
      const first = ignoredPasteboardChangeCounts.values().next().value
      ignoredPasteboardChangeCounts.delete(first)
    }
  }
}
...
export async function readPasteboardPayload(): Promise<ClipPayload | null> {
  ...
  if (ignoredPasteboardChangeCounts.has(sourceChangeCount)) {
    ignoredPasteboardChangeCounts.delete(sourceChangeCount)
    return null
  }
  try {
...
export async function writeClipToPasteboard(item: ClipItem, fullContent?: string): Promise<void> {
  ...
  await rememberSelfWrite(textToWrite)
}
